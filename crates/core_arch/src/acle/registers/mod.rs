    ...
    ($R:ident) => {
        impl super::super::sealed::Rsr for $R {
            unsafe fn __rsr(&self) -> u32 {
                let r: u32;
                asm!(concat!("mrs {},", stringify!($R)), out(reg) r, options(nostack, nomem, preserves_flags));
                r
            }
        }
    };
    ...
    ($R:ident) => {
        impl super::super::sealed::Wsr for $R {
            unsafe fn __wsr(&self, value: u32) {
                asm!(concat!("msr ", stringify!($R), ",{}"), in(reg) value, options(nostack, nomem, preserves_flags));
            }
        }
    };
}
