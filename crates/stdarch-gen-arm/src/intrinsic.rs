...
fn create_tokens(intrinsic: &Intrinsic, endianness: Endianness, tokens: &mut TokenStream) {
    ...
    let mut expressions = match endianness {
        Endianness::Little | Endianness::NA => intrinsic.compose.iter().peekable(),
        Endianness::Big => intrinsic.big_endian_compose.iter().peekable(),
    };
